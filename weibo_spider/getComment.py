...
class WeiboComment:
    ...
    def get_post(self) -> dict:
        ...
        cursor = db.cursor()
        update_query = "UPDATE post SET  name=%s, likes=%s, comment_num=%s, repost_num=%s, content=%s"

        cursor.execute(update_query, (
            post_data["name"],
            post_data["likes"],
            post_data["comment_num"],
            post_data["repost_num"],
            post_data["content"],
        ))
    ...
    def save_data(self):
        ...
        pd.io.sql.to_sql(self.data,'comments',con=engine,schema='weibo',if_exists='replace',index=False)
